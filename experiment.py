...
class Agent(snt.RNNCore):
    ...
    def _instruction(self, instruction):
        ...
        output, _ = tf.nn.dynamic_rnn(
            core, embedding, length, dtype=tf.float32)
    ...
    def _torso(self, (last_action, env_output)):
        ...
        with tf.variable_scope('convnet'):
            conv_out = frame
            for i, (num_ch, num_blocks) in enumerate([(16, 2), (32, 2), (32,
                                                                         2)]):
                # Downscale.
                conv_out = snt.Conv2D(
    ...
    def _head(self, core_output):
        policy_logits = snt.Linear(
            self._num_actions, name='policy_logits')(core_output)
        baseline = tf.squeeze(
            snt.Linear(1, name='baseline')(core_output), axis=-1)
        ...
        if is_learner:
            ...
            def make_time_major(s):
                return nest.map_structure(
                    lambda t: tf.transpose(t, [1, 0] + range(t.shape.ndims)[2:]),
                    s)
